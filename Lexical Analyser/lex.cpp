...
int main(){
     ...
     cout<<"\n\nPress Any Key to begin Code Optimization.\n";
     _getch();
     cout<<"\n\n";
     ...
     cout<<"* Takes the quadruples generated by Intermediate Code Generation.\n";
     cout<<"* Tries to improve the intermediate code by making it consume fewer resources (i.e. CPU, Memory) so that faster-running machine code will result.\n";
     cout<<"* Reduce the space consumed and increases the speed of compilation.\n";
     cout<<"* Performs Constant propagation and Constant folding in sequential blocks followed by Dead Code Elimination.\n\n";

     unordered_map<string,string> dictValues;
     ...
     for(int i = 0; i < quadlen; i++){
          ...
          if(quad[i].arg1.substr(0,2) == "ID") arg1 = whatName[quad[i].arg1];
          else if(quad[i].arg1.substr(0,3) == "NUM") arg1 = whatNumber[quad[i].arg1];
          else arg1 = quad[i].arg1;
          if(quad[i].arg2.substr(0,2) == "ID") arg2 = whatName[quad[i].arg2];
          else if(quad[i].arg2.substr(0,3) == "NUM") arg2 = whatNumber[quad[i].arg2];
          else arg2 = quad[i].arg2;

          if(quad[i].res.substr(0,2) == "ID") res = whatName[quad[i].res];
          else res = quad[i].res;
          ...
          if(quad[i].op == "+" || quad[i].op == "-" || quad[i].op == "*" || quad[i].op == "/"){
               if(isdigit(arg1[0]) && isdigit(arg2[0])){
                    ...
                    if(quad[i].op == "+") result = Arg1 + Arg2;
                    if(quad[i].op == "-") result = Arg1 - Arg2;
                    if(quad[i].op == "*") result = Arg1 * Arg2;
                    if(quad[i].op == "/") result = Arg1 / Arg2;

                    resu = to_String(result);
                    dictValues[quad[i].res] = resu;
                    cout<<"= "<<resu<<" NULL "<<res<<'\n';
                    constantFoldedList.push_back({"=",resu, "NULL",res});
               }

               else if(isdigit(arg1[0])){
                    if(dictValues.find(arg2) != dictValues.end()){
                         int Arg1 = stoi(arg1);
                         int Arg2 = stoi(dictValues[arg2]);
                         int result;
                         string resu;
                         ...
                         cout<<quad[i].op<<" "<<arg1<<" "<<arg2<<" "<<res<<"\n";
                         constantFoldedList.push_back({quad[i].op,arg1,arg2,res});
                    }
               }
               else if(isdigit(arg2[0])){
                    if(dictValues.find(arg1) != dictValues.end()){
                         int Arg2 = stoi(arg2);
                         int Arg1 = stoi(dictValues[arg1]);
                         int result;
                         string resu;
                    ...
                    if(flag1 == 1 && flag2 == 2){
                         int Arg1 = stoi(arg1Res);
                         int Arg2 = stoi(arg2Res);
                         int result;
                         string resu;
               ...
               if(isdigit(arg1[0])){
                    dictValues[quad[i].res] = arg1;
                    cout<<"= "<<arg1<<" NULL "<<res<<"\n";
                    constantFoldedList.push_back({"=",arg1,"NULL",res});
               }
               else{
                    if(dictValues.find(arg1) != dictValues.end()){
                         cout<<"= "<<dictValues[arg1]<<" NULL "<<res<<"\n";
     ...
     for(int i = 0; i < 40; i++) cout<<"~";
     cout<<"\n\n";
     for(int i = 0; i < constantFoldedList.size(); i++){
          Sleep(50);
          if(constantFoldedList[i][0] == "="){
               cout<<constantFoldedList[i][3]<<" "<<constantFoldedList[i][0]<<" "<<constantFoldedList[i][1]<<"\n";
          }
          else if(constantFoldedList[i][0] == "+" || constantFoldedList[i][0] == "-" || constantFoldedList[i][0] == "*" ||
                  constantFoldedList[i][0] == "/" || constantFoldedList[i][0] == "==" || constantFoldedList[i][0] == "<=" ||
                  constantFoldedList[i][0] == ">= " || constantFoldedList[i][0] == ">" || constantFoldedList[i][0] == "<"){
                       cout<<constantFoldedList[i][3]<<" = "<<constantFoldedList[i][1]<<" "<<constantFoldedList[i][0]<<" "<<constantFoldedList[i][2]<<"\n";
                 }
          else if(constantFoldedList[i][0] == "if" || constantFoldedList[i][0] == "label" || constantFoldedList[i][0] == "goto" ||
                   constantFoldedList[i][0] == "not"){
                        if(constantFoldedList[i][0] == "if")
                              {cout<<constantFoldedList[i][0]<<" "<<constantFoldedList[i][1]<<" goto "<<constantFoldedList[i][3]<<"\n";}
                        if(constantFoldedList[i][0] == "goto")
                              {cout<<constantFoldedList[i][0]<<" "<<constantFoldedList[i][3]<<"\n";}
                        if(constantFoldedList[i][0] == "label")
                              {cout<<constantFoldedList[i][3]<<" :\n";}
                        if(constantFoldedList[i][0] == "not")
                              {cout<<constantFoldedList[i][3]<<" = "<<constantFoldedList[i][0]<<" "<<constantFoldedList[i][1]<<"\n";}
                   }
     }
